...
このリポジトリは、これから作る個別プロジェクトへ適用するための
汎用開発テンプレートである。

その中核として、特定ベンダー向けの設定ファイルを先に増やすのではなく、
どのコーディングエージェントにも投影できる vendor-agnostic な
`Harness Kernel` を正本として設計する。

今回の目的は実装開始ではなく、既存のハーネスエンジニアリング知見を確認し、
このテンプレートで採用する前提・境界・初手の順序を明確にすることにある。
...
### 5.0 template-first の前提

この repo の主語は `Harness Kernel` 単体ではなく、
「将来のプロジェクトへ適用するための汎用テンプレート」である。

したがって、harness の適用順序は次を前提にする。

1. 対象プロジェクトの目的、ドメイン、制約、統合先を理解する
2. 調査を通じて stack と delivery model を選定する
3. `harness/manifest.yaml` に intake と stack selection を記録する
4. その project に合わせて `harness/` の artifact を調整する
5. 最後に vendor-facing adapter を emit / symlink する

この順序そのものの正本は `harness/manifest.yaml` に置き、README や
adapter file はそこへの pointer として扱う。
...
この section は kernel 自体を育てるためだけでなく、将来の project へ
template を適用するときに、どの artifact をどの責務で specialize するかを
判断しやすくするための設計図でもある。
...
- `manifest`: `specialization_record` を含む committed specialization record と、この repo が何を作るかの宣言
...
- `docs/adr/`: design and workflow decisions の decision log
...
この repo は空の状態から始まったため、まず template の主語と capability vocabulary、
rollout order を文書で固定し、その後に最小 schema と matrix を作る順序が最も安全だった。
...
1. `harness/manifest.yaml` の `specialization_record` を target project ごとに埋められるよう運用を固める
2. `harness/compatibility-matrix.yaml` の coverage を広げる
3. project-specific oracle commands を specialization 後に実コマンドへ置き換える
